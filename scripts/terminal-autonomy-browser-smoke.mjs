...
try {
  for (const provider of ['codex', 'claude', 'qodercli']) for (const width of [390, 1365]) {
    ...
    if (simpleMode) {
      ...
      await a.click(); assert.equal(await dialog.getByRole('textbox', { name: '目标：自定义回答' }).inputValue(), '修复终端宽度，回归通过');
      await dialog.getByRole('button', { name: '开始自主执行' }).click();
      await page.waitForFunction(() => document.querySelector('#terminalAutonomyStatus').textContent.includes('待执行'));
      await fixture.autonomy.tick(); assert.equal(fixture.sent.length, 1);
      const id = fixture.autonomy.snapshot(fixture.target).id;
      fixture.sessions[0].hasRunningProcess = true;
      await a.click(); await page.waitForFunction(() => document.querySelector('#terminalAutonomyStatus').textContent.includes('总结退出中'));
      ...
      await page.waitForFunction(() => document.querySelector('#terminalAutonomyStatus').textContent.includes('已退出'));
      await a.click(); await dialog.getByRole('heading', { name: '设置自主目标' }).waitFor();
