...
test('the input bar, when enabled, keeps typing local and hands whitelisted keys to the CLI', () => {
  // 逐字符直通让打字延迟等于 RTT。开启本地输入后收进输入条, 回车才发一次;
...
test('the terminal behaves like a terminal by default; local input is opted into', () => {
  // 默认逐字符直达 CLI, 和原生终端一致。本地输入省掉每键一次往返, 但代价是 CLI 收不到
  // 逐键输入, 所以它是设置里显式开启的, 不是默认。
  assert.match(appJs, /LOCAL_INPUT_STORAGE_KEY = 'codeck-local-input'/);
  assert.match(appJs, /if \(writable && localInputEnabled\(\)\) openTerminalComposer\(\)/);
  assert.doesNotMatch(html, /id="inputModeButton"/);
  assert.match(html, /id="localInputToggle"/);
  // 无痕模式下 localStorage 会抛, 读不能让终端页挂掉。
  assert.match(appJs, /try \{ return localStorage\.getItem\(LOCAL_INPUT_STORAGE_KEY\)[\s\S]{0,60}?catch \{ return false; \}/);
  // 显隐输入条会改变终端高度, 必须显式重排, 不能指望 ResizeObserver 的时序。
  assert.match(appJs, /fitTerminalView\(\);\n  focusTerminalInput\(\)/);
});
