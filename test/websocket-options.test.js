...
import { AGENT_WEBSOCKET_OPTIONS, TERMINAL_WEBSOCKET_OPTIONS } from '../src/websocket-options.js';
...
test('terminal WebSocket negotiates bounded per-message compression for large repaints', async (t) => {
  assert.equal(TERMINAL_WEBSOCKET_OPTIONS.perMessageDeflate.threshold, 4 * 1024);
  assert.equal(TERMINAL_WEBSOCKET_OPTIONS.perMessageDeflate.serverNoContextTakeover, true);
  assert.equal(TERMINAL_WEBSOCKET_OPTIONS.perMessageDeflate.clientNoContextTakeover, true);

  const server = http.createServer();
  const wss = new WebSocketServer({ server, ...TERMINAL_WEBSOCKET_OPTIONS });
  let client;
  t.after(async () => {
    client?.terminate();
    await new Promise((resolve) => wss.close(resolve));
  ...
  assert.match(client.extensions, /permessage-deflate/);
  assert.match(peer.extensions, /permessage-deflate/);
  const payload = '\x1b[2J\x1b[Hterminal repaint\n'.repeat(8_000);
  const received = once(client, 'message');
  peer.send(payload);
  const [message] = await received;
  assert.equal(String(message), payload);
