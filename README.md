...
Remote 输入区旁的圈 A（`Ⓐ`）用于自主模式。点击后，Agent 会通过对话询问目标、验收标准、预算和偏好；确认方案并回复“开始”后，服务端逐轮推进。按钮只显示符号、轮数和必要状态，不增加面板。再次点击暂停后续轮次；要立即打断当前执行，使用原有停止按钮。在输入框补充方向后确认“继续”，或直接说“只修改后端，继续”，可保留已用轮数调整目标；增加预算仍需确认。
