...
配置和讨论新目标不会取消旧任务；确认执行后进入“切换中”，先停止旧任务，再检查同一个 Agent/pane 已结束执行，才发送新目标。停止失败、超时、身份改变或中途暂停不会启动新目标，也不消耗轮数；服务重启不会重放取消操作。当前只自动停止前台 Agent 工作；检测到后台执行时暂停切换，提示先停止后台任务后再次确认，不擅自杀进程或让新旧任务并行。
