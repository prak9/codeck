...
Remote 的乐观回显会标注“等待 Agent 确认”，直到真实对话记录出现。发送到 Codex 后若无法确认文字已提交，页面会保留草稿并阻止同一次输入重复投递；请从右上角切换到终端模式，检查文字是否仍在 CLI 输入框，再决定是否按 Enter 提交，不要重新粘贴整条消息。
...
Qoder 普通单行文字和回车通过同一份原始终端输入发送，避免忙碌时粘贴与回车被合并成换行；多行输入保留括号粘贴。Codeck 也会核对 Qoder 自己的输入日志：确认收到后显示“Qoder 已接收”，不会因为排队或 steering 尚未产生独立对话记录而超时变成未知。收到不等于执行完成；真实记录出现后才替换这条回显，确认延迟期间编辑的新草稿不会被清空。
