...
Remote 的 `?` 和圈 A（`Ⓐ`）相邻排列。A 用于自主模式：Agent 根据已有对话，通过弹窗选择题补齐目标、验收标准、预算和偏好，支持自定义答案；随后展示具体目标和预算，选择“按此目标开始”才逐轮执行，也可调整或暂不开始。关闭弹窗不代表批准，点击 A 可重新打开。按钮只显示符号、轮数和必要状态，不增加面板。执行中再次点击 A 暂停后续轮次；要立即打断当前执行，使用原有停止按钮。在输入框补充方向后确认“继续”，或直接说“只修改后端，继续”，可保留已用轮数调整目标；增加预算仍需确认。
